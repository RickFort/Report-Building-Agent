...
def create_calculator_tool(logger: ToolLogger):
    ...
    def is_safe_expression(expr: str) -> bool:
        '''Function that evaluate only numeber and mathematical expression'''
        if not re.compile(r"^[0-9+\-*/().\s*]+$").match(expr):
            return False
        return True
    ...
    def calculator(expression: str) -> str:
        """
        Evaluate a mathematical expression safely using pure Python.
        ...
        Args:
            expression (str): Arithmetic expression, e.g. "2 + 3 * (4 - 1)"
        ...
        try:
            ...
            if not is_safe_expression(expression):
                error_msg = "Invalid or unsafe mathematical expression."
                logger.log_tool_use(
                    "calculator",
            ...
            # Eval in namespace vuoto (più sicuro possibile senza AST)
