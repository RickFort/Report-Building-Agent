...
class UserIntent(BaseModel):
    ...
    intent_type: Literal["qa", "summarization", "calculation", "unknown"] = Field(
        description="The classified intent type: qa, summarization, calculation, or unknown"
        )
    confidence: float = Field(
        ge=0.0
        ,le=1.0
    ...
    reasoning: str = Field(
        description="Explanation of why this intent was chosenbased on the user input and context"
        )
...
class SessionState(BaseModel):
    ...
    session_id: str
    user_id: str
    conversation_history: List[TypedDict] = Field(
        default_factory=lambda: list
        )
    document_context: List[str] = Field(
        default_factory=lambda: list
        , description="Active document IDs"
        )
