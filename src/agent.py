...
def calculation_agent(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    calculator_tool = [tool for tool in tools if tool.name in ["document_reader","calculator"]]
    ...
    result, tools_used = invoke_react_agent(CalculationResponse, messages, llm, calculator_tool)
